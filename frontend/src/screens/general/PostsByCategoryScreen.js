...
const PostsByCategoryScreen = (props) => {
  const { categoryName: categoryNameProp, posts } = props || {};
  const classes = useStyles();
  const params = useParams();
  const categoryName = categoryNameProp ?? params?.categoryName;
  ...
  const { data: queriedPosts, isLoading: queriedIsLoading, isFetching: queriedIsFetching, isError: queriedIsError } = useQuery({
    queryKey: [POST_DATA, { categoryName }],
    queryFn: ({ queryKey }) => getPostByCategoryName(queryKey[1]),
